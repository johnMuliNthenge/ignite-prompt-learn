...
export default function LibraryCategoryManagement() {
  ...
  const fetchCategories = async () => {
    const { data } = await supabase.from('library_categories').select('*').order('name');
  ...
  const handleSubmit = async () => {
    try {
      if (editing) {
        const { error } = await supabase.from('library_categories').update({
  ...
  return (
    <ProtectedPage moduleCode="library.categories" title="Library Categories">
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold">Library Categories</h1><p className="text-muted-foreground">Organize books into categories</p></div>
          <Dialog open={dialogOpen} onOpenChange={o => { setDialogOpen(o); if (!o) resetForm(); }}>
            <DialogTrigger asChild><Button><Plus className="mr-2 h-4 w-4" />Add Category</Button></DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{editing ? 'Edit Category' : 'Add Category'}</DialogTitle>
                <DialogDescription>{editing ? 'Update the category details' : 'Create a new library category'}</DialogDescription>
              </DialogHeader>
