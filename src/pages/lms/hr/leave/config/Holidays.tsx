...
export default function Holidays() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      if (editingItem) {
        const { error } = await supabase.from('hr_holidays').update(data).eq('id', editingItem.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('hr_holidays').insert(data);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      toast({ title: `Holiday ${editingItem ? 'updated' : 'created'} successfully` });
  ...
  const handleClose = () => {
    setOpen(false);
    setEditingItem(null);
    setFormData({ name: '', date: '', is_recurring: false, is_active: true });
  };
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Manage public and company holidays</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <form onSubmit={handleSubmit}>
              <DialogHeader>
                <DialogTitle>{editingItem ? 'Edit' : 'Add'} Holiday</DialogTitle>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="space-y-2">
                  <Label htmlFor="name">Name *</Label>
                  <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g., Christmas Day" />
                </div>
                <div className="space-y-2">
                ...
                <div className="flex items-center space-x-2">
                  <Switch id="is_recurring" checked={formData.is_recurring} onCheckedChange={(checked) => setFormData({ ...formData, is_recurring: checked })} />
