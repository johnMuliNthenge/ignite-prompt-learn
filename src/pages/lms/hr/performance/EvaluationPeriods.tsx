...
export default function EvaluationPeriods() {
  ...
  const { data: items, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('hr_performance_periods').select('*').order('start_date', { ascending: false });
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      if (editingItem) {
        const { error } = await supabase.from('hr_performance_periods').update(data).eq('id', editingItem.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('hr_performance_periods').insert(data);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      toast({ title: `Evaluation period ${editingItem ? 'updated' : 'created'} successfully` });
      queryClient.invalidateQueries({ queryKey: ['hr-performance-periods'] });
      handleClose();
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('hr_performance_periods').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Define performance review cycles</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <form onSubmit={handleSubmit}>
              <DialogHeader>
                <DialogTitle>{editingItem ? 'Edit' : 'Add'} Evaluation Period</DialogTitle>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="space-y-2">
                  <Label htmlFor="name">Name *</Label>
                  <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g., Q1 2024 Review" />
                </div>
                <div className="space-y-2">
      ...
      <Card>
        <CardContent className="pt-6">
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">No evaluation periods found</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
