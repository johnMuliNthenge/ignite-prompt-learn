...
export default function Skills() {
  ...
  const { data: items, isLoading } = useQuery({
    queryKey: ['hr-skills'],
    queryFn: async () => {
      const { data, error } = await supabase.from('hr_skills').select('*').order('name');
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      if (editingItem) {
        const { error } = await supabase.from('hr_skills').update(data).eq('id', editingItem.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('hr_skills').insert(data);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      toast({ title: `Skill ${editingItem ? 'updated' : 'created'} successfully` });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('hr_skills').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div><h1 className="text-3xl font-bold">Skills</h1><p className="text-muted-foreground">Manage employee skills</p></div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button onClick={() => handleClose()}><Plus className="mr-2 h-4 w-4" /> Add Skill</Button></DialogTrigger>
          <DialogContent>
