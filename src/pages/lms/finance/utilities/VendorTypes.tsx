...
export default function VendorTypes() {
  ...
  const [editingItem, setEditingItem] = useState<VendorType | null>(null);
  ...
  const fetchData = async () => {
    setLoading(true);
    const { data } = await supabase.from('vendor_types').select('*').order('name');
  ...
  const handleSubmit = async () => {
    ...
    if (editingItem) {
      const { error } = await supabase.from('vendor_types').update(payload).eq('id', editingItem.id);
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await supabase.from('vendor_types').delete().eq('id', id);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Vendor Types</h1>
          <p className="text-muted-foreground">Categorize your suppliers and vendors</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) { setEditingItem(null); setFormData({ name: '', description: '', is_active: true }); } }}>
          <DialogTrigger asChild>
            <Button><Plus className="mr-2 h-4 w-4" /> Add Vendor Type</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editingItem ? 'Edit' : 'Add'} Vendor Type</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div className="space-y-2">
                <Label>Name *</Label>
                <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g., Supplier, Service Provider" />
              </div>
              <div className="space-y-2">
      ...
      <Card>
        <CardHeader><CardTitle>Vendor Types</CardTitle></CardHeader>
        <CardContent>
          {loading ? (
            ...
            <Table>
              ...
              <TableBody>
                {types.length === 0 ? (
                  <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground">No vendor types found</TableCell></TableRow>
                ) : (
                  types.map((item) => (
